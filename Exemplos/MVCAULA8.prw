...
±±ºPrograma  ³MVCAULA8  ºAutor  ³Fernando W S Furtadoº Data ³  05/27/12   º±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³MVCAULA7  ºAutor  ³Microsiga           º Data ³  05/30/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
oAux    := oModel:GetModel('ID_MODEL_FLD_MVCAULA2')
...
If lRet
	For nI := 1 To Len( aCpoMaster )
		...
		If ( nPos := aScan( aAux, { |x| AllTrim( x[3] ) ==  AllTrim( aCpoMaster[nI][1] ) } ) ) > 0
			...
			If !( lAux := oModel:SetValue( 'ID_MODEL_FLD_MVCAULA2', aCpoMaster[nI][1], aCpoMaster[nI][2] ) )
...
If lRet
	...
	oAux     := oModel:GetModel( 'ID_MODEL_GRD_MVCAULA2' )
	...
	For nI := 1 To Len( aCpoDetail )
		...
		// ATENCAO: O itens são criados em uma estrura de grid (FORMGRID), portanto já 
		 //é criada uma primeira linha
		...
		// Verifica se os campos passados existem na estrutura de item
			If ( nPos := aScan( aAux, { |x| AllTrim( x[3] ) ==  ;
			                                 AllTrim( aCpoDetail[nI][nJ][1] ) } ) ) > 0

				If !( lAux := oModel:SetValue( 'ID_MODEL_GRD_MVCAULA2',;
